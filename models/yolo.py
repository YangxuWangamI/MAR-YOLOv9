...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        # self.dyhead = nn.Sequential(*[DyHeadBlock(ch[0]) for i in range(2)])  # Added when dyhead is used. range(2) 2表示深度
    ...
    def forward(self, x):
        # for dyhead_layer in self.dyhead:  # Added when dyhead is used
        #     x = dyhead_layer(x)  # Added when dyhead is used
    ...
    def bias_init(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            ...
            b[-1].bias.data[:m.nc] = math.log(5 / m.nc / (608 / s) ** 2)  # cls (.01 objects, 80 classes, 640 img)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            RepNCSPELAN4, SPPELAN, ShuffleAttention, CBAM}:
            c1, c2 = ch[f], args[0]
