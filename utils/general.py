...
def non_max_suppression(
    ...
    if isinstance(prediction, (list, tuple)):  # YOLO model in validation model, output = (inference_out, loss_out)
        prediction = prediction[0][0]  # [DualDDetect val use] select only inference output
        # prediction = prediction[0]   # [Detect val use & use when train] select only inference output
